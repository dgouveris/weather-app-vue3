  ...
  <main class="container text-white">
    <div class="pt-4 mb-8 relative">
      ...
      <ul v-if="mapboxSearchResults"
       class="absolute bg-weather-secondary text-white w-full 
      shadow-md py-2 px-1 top-[66px]">
        <p v-if="searchError">Sorry, something went wrong, please try again.</p>
        <p v-if="!searchError && mapboxSearchResults.length === 0">No results match your query, try a differernt term.</p>
        <template v-else>
  ...
  queryTimeout.value = setTimeout(async () => {
    if(searchQuery.value !== "")
    {
      try{
        ...
        ${searchQuery.value}.json?access_token=${mapboxAPIKey}&types=place&language=el`);
